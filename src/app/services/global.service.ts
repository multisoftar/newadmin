...
export class GlobalService {
  ...
  moduloSelected: { name: string,  images: any[] } = { name: "Seleccione un modulo",  images: [] };
  integrationSelected: { name: string,  images: any[] } = { name: "Seleccionar",  images: [] };
  rubroSelected: { id:string, name: string,  images: any[] } = { name: "Seleccionar",  images: [] , id:""};
  ...
  updateRubro(car :RubroInterface, id: string){
		// let token = this.authService.getToken();
		const url_api=	this.yeoman.origin.restUrl+`/api/categories/${id}`;
		return this.http
		.put<RubroInterface>(url_api, car)
		.pipe(map(data => data));
	}
